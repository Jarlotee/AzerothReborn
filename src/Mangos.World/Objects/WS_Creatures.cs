...
public class WS_Creatures
{
    public class CreatureObject : WS_Base.BaseUnit, IDisposable
    {
        ...
        public int NPCTextID
        {
            get
            {
                checked
                {
                    return WorldServiceLocator.WSDBCDatabase.CreatureGossip.ContainsKey(GUID - WorldServiceLocator.GlobalConstants.GUID_UNIT)
                        ? WorldServiceLocator.WSDBCDatabase.CreatureGossip[GUID - WorldServiceLocator.GlobalConstants.GUID_UNIT]
                        : 16777215;
                }
            }
        }
        ...
        public void FillAllUpdateFlags(ref Packets.UpdateClass Update)
        {
            ...
            checked
            {
                Update.SetUpdateFlag(23 + WorldServiceLocator.WorldServer.CREATURESDatabase[ID].ManaType, Mana.Current);
                Update.SetUpdateFlag(28, Life.Maximum);
                Update.SetUpdateFlag(29 + WorldServiceLocator.WorldServer.CREATURESDatabase[ID].ManaType, Mana.Maximum);
        ...
        public void MoveToInstant(float x, float y, float z, float o)
        {
            ...
            if (SeenBy.Count > 0)
            {
                ...
                packet.AddInt32(0);
                packet.AddInt32(WorldServiceLocator.NativeMethods.timeGetTime(""));
        ...
        public void SetToRealPosition(bool Forced = false)
        {
            ...
            {
                var timeDiff = checked(WorldServiceLocator.NativeMethods.timeGetTime("") - LastMove);
        ...
        public int MoveTo(float x, float y, float z, float o = 0f, bool Running = false)
        {
            ...
            catch (Exception projectError)
            {
                ...
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "MoveTo:SeenBy Failed");
                ProjectData.ClearProjectError();
            }
            var TimeToMove = 1;
            ...
            checked
            {
                try
                {
                    ...
                    var moveDist = WorldServiceLocator.WSCombat.GetDistance(positionX, x, positionY, y, positionZ, z);
                    if (Flying)
                    {
                    ...
                    OldY = positionY;
                    OldZ = positionZ;
                    LastMove = WorldServiceLocator.NativeMethods.timeGetTime("");
                ...
                catch (Exception ex2)
                {
                    ProjectData.SetProjectError(ex2);
                    var ex = ex2;
                    WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "MoveTo:Main Failed - {0}", ex.Message);
                    ProjectData.ClearProjectError();
                }
                finally
                {
        ...
        public bool GenerateLoot(ref WS_PlayerData.CharacterObject Character, LootType LootType)
        {
            ...
            checked
            {
                ...
                {
                    Loot.Money = WorldServiceLocator.WorldServer.Rnd.Next((int)CreatureInfo.MinGold, (int)(CreatureInfo.MaxGold + 1L));
                }
        ...
        public int CastSpell(int SpellID, WS_Base.BaseUnit Target)
        {
            ...
            if (WorldServiceLocator.WSCombat.GetDistance(this, Target) > WorldServiceLocator.WSSpells.SPELLs[SpellID].GetRange)
            {
                return -1;
            }
            WS_Spells.SpellTargets Targets = new();
        ...
        public int CastSpell(int SpellID, float x, float y, float z)
        {
            ...
            if (WorldServiceLocator.WSCombat.GetDistance(this, x, y, z) > WorldServiceLocator.WSSpells.SPELLs[SpellID].GetRange)
            {
                return -1;
            }
            WS_Spells.SpellTargets Targets = new();
        ...
        public void SendChatMessage(string Message, ChatMsg msgType, LANGUAGES msgLanguage, ulong SecondGUID = 0uL)
        {
            ...
            {
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "Creature.SendChatMessage() must not handle this chat type!");
            }
        ...
        {
            Level = checked((byte)WorldServiceLocator.WorldServer.Rnd.Next(WorldServiceLocator.WorldServer.CREATURESDatabase[ID].LevelMin, WorldServiceLocator.WorldServer.CREATURESDatabase[ID].LevelMax));
            ...
            checked
            {
                ...
                if (Operators.CompareString(WorldServiceLocator.WorldServer.CREATURESDatabase[ID].AIScriptSource, "", TextCompare: false) != 0)
                {
                    aiScript = (WS_Creatures_AI.TBaseAI)WorldServiceLocator.WorldServer.AI.InvokeConstructor(WorldServiceLocator.WorldServer.CREATURESDatabase[ID].AIScriptSource, new object[1]
                    {
                            this
                    });
                }
                ...
                {
                    ScriptedObject tmpScript = new("scripts\\creatures\\" + WorldServiceLocator.Functions.FixName(Name) + ".vb", "", InMemory: true);
                    aiScript = (WS_Creatures_AI.TBaseAI)tmpScript.InvokeConstructor("CreatureAI_" + WorldServiceLocator.Functions.FixName(Name).Replace(" ", "_"), new object[1]
                    {
                            this
                    });
                    tmpScript.Dispose();
        ...
        public CreatureObject(ulong GUID_, DataRow infoRow = null)
        {
            ...
            if (infoRow == null)
            {
                ...
                WorldServiceLocator.WorldServer.WorldDatabase.Query($"SELECT * FROM creature LEFT OUTER JOIN game_event_creature ON creature.guid = game_event_creature.guid WHERE creature.guid = {GUID_};", ref MySQLQuery);
                if (MySQLQuery.Rows.Count <= 0)
                {
                    WorldServiceLocator.WorldServer.Log.WriteLine(LogType.FAILED, "Creature Spawn not found in database. [GUID={0:X}]", GUID_);
                    return;
                }
            ...
            WorldServiceLocator.WorldServer.WorldDatabase.Query($"SELECT * FROM spawns_creatures_addon WHERE spawn_id = {GUID_};", ref AddonInfoQuery);
            ...
            catch (Exception ex2)
            {
                ProjectData.SetProjectError(ex2);
                var ex = ex2;
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "WS_Creatures:New failed - Guid: {1}  {0}", ex.Message, GUID_);
                ProjectData.ClearProjectError();
            }
        }
        ...
        public CreatureObject(ulong GUID_, int ID_)
        {
            ...
            catch (Exception ex2)
            {
                ProjectData.SetProjectError(ex2);
                var ex = ex2;
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "WS_Creatures:New failed - Guid: {1} ID: {2}  {0}", ex.Message, GUID_, ID_);
                ProjectData.ClearProjectError();
            }
        }
        ...
        public CreatureObject(int ID_)
        {
            ...
            catch (Exception ex2)
            {
                ProjectData.SetProjectError(ex2);
                var ex = ex2;
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "WS_Creatures:New failed - Guid: {1} ID: {2}  {0}", ex.Message, ID_);
                ProjectData.ClearProjectError();
            }
        }
        ...
        public CreatureObject(int ID_, float PosX, float PosY, float PosZ, float Orientation_, int Map, int Duration = 0)
        {
            ...
            catch (Exception ex2)
            {
                ProjectData.SetProjectError(ex2);
                var ex = ex2;
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "WS_Creatures:New failed - Guid: {1} ID: {2} Map: {3}  {0}", ex.Message, GUID, ID_, Map);
                ProjectData.ClearProjectError();
            }
        }
        ...
        protected virtual void Dispose(bool disposing)
        {
            if (!_disposedValue)
            {
                ...
                catch (Exception ex2)
                {
                    ProjectData.SetProjectError(ex2);
                    var ex = ex2;
                    WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "WS_Creatures:Dispose failed -  {0}", ex.Message);
                    ProjectData.ClearProjectError();
                }
            }
            _disposedValue = true;
        ...
        {
            if (decimal.Compare(new decimal(SummonedBy), 0m) > 0 && WorldServiceLocator.CommonGlobalFunctions.GuidIsPlayer(SummonedBy) && WorldServiceLocator.WorldServer.CHARACTERs.ContainsKey(SummonedBy) && WorldServiceLocator.WorldServer.CHARACTERs[SummonedBy].NonCombatPet != null && WorldServiceLocator.WorldServer.CHARACTERs[SummonedBy].NonCombatPet == this)
            {
        ...
        public void AddToWorld()
        {
            ...
            catch (Exception ex2)
            {
                ProjectData.SetProjectError(ex2);
                var ex = ex2;
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "WS_Creatures:AddToWorld failed - Guid: {1} ID: {2}  {0}", ex.Message);
                ProjectData.ClearProjectError();
                return;
            }
            short i = -1;
        ...
        public void RemoveFromWorld()
        {
            ...
            {
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "WS_Creatures:RemoveFromWorld - Unable to remove creatue from world, Remove again  {0}", ex.Message);
                WorldServiceLocator.WSMaps.Maps[MapID].Tiles[CellX, CellY].CreaturesHere.Remove(GUID);
            }
        }
        ...
        public void MoveCell()
        {
            ...
            catch (Exception ex2)
            {
                ProjectData.SetProjectError(ex2);
                var e = ex2;
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "WS_Creatures:MoveCell - Creature outside of map bounds, Resetting  {0}", e.Message);
                try
                {
                    aiScript.Reset();
                }
                catch (Exception ex3)
                {
                    ...
                    var ex = ex3;
                    WorldServiceLocator.WorldServer.Log.WriteLine(LogType.FAILED, "WS_Creatures:MoveCell - Couldn't reset creature outside of map bounds, Disposing  {0}", ex.Message);
                    aiScript.Dispose();
                    ProjectData.ClearProjectError();
                }
                ProjectData.ClearProjectError();
    ...
    public class NPCText
    {
        ...
        public NPCText(int _TextID)
        {
            ...
            TextID = _TextID;
            DataTable MySQLQuery = new();
            WorldServiceLocator.WorldServer.WorldDatabase.Query($"SELECT * FROM npc_text WHERE ID = {TextID};", ref MySQLQuery);
            checked
            {
                if (MySQLQuery.Rows.Count > 0)
                {
    ...
    public void On_CMSG_CREATURE_QUERY(ref Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        checked
        {
            ...
            try
            {
                if (!WorldServiceLocator.WorldServer.CREATURESDatabase.ContainsKey(CreatureID) && CreatureID != 0 && CreatureGUID != 0)
                {
                    WorldServiceLocator.WorldServer.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CREATURE_QUERY [Creature {2} not loaded.]", client.IP, client.Port, CreatureID);
            ...
            catch (Exception ex2)
            {
                ProjectData.SetProjectError(ex2);
                var ex = ex2;
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.FAILED, "Unknown Error: Unable to find CreatureID={0} in database. {1}", CreatureID, ex.Message);
                ProjectData.ClearProjectError();
            }
        }
    }
    ...
    public void On_CMSG_NPC_TEXT_QUERY(ref Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        checked
        {
            if (packet.Data.Length - 1 >= 17)
            {
                ...
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_NPC_TEXT_QUERY [TextID={2}]", client.IP, client.Port, TextID);
    ...
    public void On_CMSG_GOSSIP_HELLO(ref Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        ...
        WorldServiceLocator.WorldServer.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GOSSIP_HELLO [GUID={2:X}]", client.IP, client.Port, GUID);
        if (!WorldServiceLocator.WorldServer.WORLD_CREATUREs.ContainsKey(GUID) || WorldServiceLocator.WorldServer.WORLD_CREATUREs[GUID].CreatureInfo.cNpcFlags == 0)
        {
            WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "[{0}:{1}] Client tried to speak with a creature that didn't exist or couldn't interact with. [GUID={2:X}  ID={3}]", client.IP, client.Port, GUID, WorldServiceLocator.WorldServer.WORLD_CREATUREs[GUID].ID);
        }
        else
        {
            ...
            catch (Exception ex2)
            {
                ProjectData.SetProjectError(ex2);
                var ex = ex2;
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.CRITICAL, "Error in gossip hello.{0}{1}", Environment.NewLine, ex.ToString());
                ProjectData.ClearProjectError();
            }
        }
    }
    ...
    public void On_CMSG_GOSSIP_SELECT_OPTION(ref Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        ...
        WorldServiceLocator.WorldServer.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_GOSSIP_SELECT_OPTION [SelOption={3} GUID={2:X}]", client.IP, client.Port, GUID, SelOption);
        if (!WorldServiceLocator.WorldServer.WORLD_CREATUREs.ContainsKey(GUID) || WorldServiceLocator.WorldServer.WORLD_CREATUREs[GUID].CreatureInfo.cNpcFlags == 0)
        {
            WorldServiceLocator.WorldServer.Log.WriteLine(LogType.WARNING, "[{0}:{1}] Client tried to speak with a creature that didn't exist or couldn't interact with. [GUID={2:X}  ID={3}]", client.IP, client.Port, GUID, WorldServiceLocator.WorldServer.WORLD_CREATUREs[GUID].ID);
        }
        else
        {
            ...
            WorldServiceLocator.WorldServer.CREATURESDatabase[WorldServiceLocator.WorldServer.WORLD_CREATUREs[GUID].ID].TalkScript.OnGossipSelect(ref client.Character, GUID, SelOption);
        }
    }
    ...
    public void On_CMSG_SPIRIT_HEALER_ACTIVATE(ref Packets.PacketClass packet, ref WS_Network.ClientClass client)
    {
        checked
        {
            ...
            packet.GetInt16();
            var GUID = packet.GetUInt64();
            WorldServiceLocator.WorldServer.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_SPIRIT_HEALER_ACTIVATE [GUID={2}]", client.IP, client.Port, GUID);
            try
            {
                byte i = 0;
                do
            ...
            catch (Exception ex2)
            {
                ProjectData.SetProjectError(ex2);
                var ex = ex2;
                WorldServiceLocator.WorldServer.Log.WriteLine(LogType.FAILED, "Error activating spirit healer: {0}", ex.ToString());
                ProjectData.ClearProjectError();
            }
